    ...
    class FileUpdater < Dependabot::FileUpdaters::Base
      ...
      def update_lockfiles_for_buildfiles(files, buildfiles_processed)
        ...
        buildfiles_processed.each_value do |buildfile|
          ...
          # Track original lockfile names in this root before update
          original_lockfile_names = lockfile_updater.lockfiles_for_root(root_dir).to_set(&:name)

          updated_files = lockfile_updater.update_lockfiles(buildfile)
          updated_lockfile_names = updated_files.select { |f| f.name.end_with?(".lockfile") }.to_set(&:name)

          # Remove lockfiles that were deleted by the updater (stale files) BEFORE replacing
          stale_lockfiles = original_lockfile_names - updated_lockfile_names
          stale_lockfiles.each do |stale_name|
            files.delete_if { |f| f.name == stale_name }
          end
